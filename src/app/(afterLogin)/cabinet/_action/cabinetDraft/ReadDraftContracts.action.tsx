...
export default function ReadDraftContractsAction() {
  ...
  return (
    <section className="w-full">
      <div className="border-custom-slate-border-side flex flex-col items-start justify-between gap-3 rounded-2xl border bg-white p-3.5 sm:flex-row sm:items-center">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <span className="text-text-sub font-semibold">
              (대상 강사: {drafts[0].instructorName} · {drafts[0].instructorPhone} · 작성일: {drafts[0].createdAt})
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto">
          <button
            ...
            className="hover:bg-rose-50 border-rose-200 cursor-pointer rounded-xl border bg-white px-2.5 py-1 text-[11px] font-bold text-rose-600 transition-all"
          >
            삭제
          </button>
          <button
            type="button"
            onClick={handleEdit}
            className="bg-custom-indigo hover:bg-custom-indigo-hover active:scale-95 cursor-pointer rounded-xl px-3.5 py-1 text-[11px] font-bold text-white transition-all"
          >
            <span>이어 작성하기</span>
          </button>
        </div>
