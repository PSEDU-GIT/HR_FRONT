      ...
      {instructorList.map((item) => {
        ...
        return (
          <button
            ...
            className={cx(
              'flex w-full items-center justify-start rounded-lg px-3 py-2 text-left text-xs font-semibold transition-all duration-150',
              isDisabled
                ? 'opacity-45 cursor-not-allowed pointer-events-none select-none bg-transparent text-text-side'
                : isSelected
                  ? 'bg-white border-custom-slate-border text-custom-indigo border shadow-2xs cursor-pointer'
                  : 'hover:bg-custom-slate-hover text-text-sub hover:text-text-title border border-transparent cursor-pointer',
            )}
          >
            <span>{item.name}</span>
          </button>
